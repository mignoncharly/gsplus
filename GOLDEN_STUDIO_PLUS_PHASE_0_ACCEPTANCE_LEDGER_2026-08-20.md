...
Production revision: `c3888fa5606fc9a159f12a8e5c5ace04bd06fcfd`
...
|---|---|---|---|---|---|---|
| `MAIL-01` | Deployed; awaiting external journey proof | `add_contact_locale` applied 20 August 2026 | Backend 176/176; frontend 95/95; translation audit passed Chromium/WebKit | Missing: external FR/EN journey replay | Pending | Roll back app first; retain locale columns/evidence |
...
| `CAL-03` | Implemented and deployed in Phase 7; final provider-event proof pending | None | Cal.com integration 14/14: FR/EN snapshot language, supported title/notes fields, Douala times, payment label, admin URL, metadata limits, and unrestricted-note exclusion | Live API/event-type capability check passed for event type `5733625`; isolated create/inspect/cancel awaits dedicated QA destinations | Pending | Revert the Phase 7 payload fields while retaining reconciliation metadata; never send customer/internal free text |
...
| `UI-01` | Open shared WhatsApp consent readability | `add_whatsapp_marketing_consent` applied 20 August 2026 | Broad axe fails; dedicated four-form FR/EN contrast matrix absent | None | Pending | Keep consent off by default and transactional/marketing purposes separate |
...
| `UI-03` | Deployed; targeted production replay passed, broader device proof pending | None | WhatsApp FAB viewport/collision tests pass in both engines | Missing broader device/keyboard production matrix | Pending | Feature-gate external channel independently of FAB |
...
| `WA-E2E-01` | Operationally blocked: provider not configured/proven | Consent migration applied 20 August 2026 | Backend consent/outbox tests pass locally; no external delivery proof | Missing Meta configuration, consent/no-consent delivery, retry/no-duplicate proof | Pending | Keep delivery feature-gated off until credentials and proof exist |

## Release-wide evidence

| Evidence | Status | Reference |
|---|---|---|
| Reviewed patch boundary | Complete for Phase 0 release | `GOLDEN_STUDIO_PLUS_PHASE_0_WORKSPACE_CHANGE_CATALOGUE_2026-08-20.md`; all ten pre-existing deletions excluded |
| Immutable candidate revision | Present and pushed | `c3888fa5606fc9a159f12a8e5c5ace04bd06fcfd` on `origin/codex/phase0-release-20260820` |
| Database migration proof | Passed | Configured target is 32/32; schema up to date |
...
| Production frontend identity | Proven | Current `index-DM1CXvtx.js`, SHA-256 recorded in manifest |
| Production backend identity | Proven | Commit `c3888fa`; service active since 20 August 2026 10:45:27 UTC; HTTPS health 200 |
| Production browser/accessibility | Passed | Targeted Chromium/WebKit scenario evidence 50/50; dedicated axe 2/2 |
