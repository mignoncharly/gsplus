...
const AdminLeadsPanel = ({ leads, cardRefs, onStatusChange }) => (
  <>
    ...
    <div className="admin-card">
      {leads.length === 0 && <p style={{ color: 'var(--dark-muted)', textAlign: 'center', padding: '2rem 0', margin: 0 }}>Aucune demande reçue pour le moment.</p>}
      {leads.map((lead) => (
        <div key={lead.id} className="admin-lead-card" data-lead-reference={lead.reference} tabIndex="-1" ref={(element) => { if (element) cardRefs.current.set(lead.reference, element); else cardRefs.current.delete(lead.reference); }}>
          <div className="admin-lead-header">
            <div><h3>{lead.company || lead.name}</h3><code className="admin-public-reference">{lead.reference}</code><p style={{ color: 'var(--dark-muted)', fontSize: '0.9rem', margin: 0 }}>{lead.email || "Pas d'email"} | {lead.phone || 'Pas de téléphone'}</p></div>
